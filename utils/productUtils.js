...
/**
 * Get the actual sales count for a product
 * @param {Object} product - The product object
 * @returns {string|null} - Formatted sold count like "5 sold", or null if 0
 */
export function getProductSoldCount(product) {
  ...
  // Try different possible properties where sales count might be stored
  const salesCount = product.sales_count || product.sold_count || product.sales || 0;
  
  if (salesCount === 0) {
    return null; // Empty so it doesn't display "0 sold"
  }
