...
import { uploadFileToStorage, deleteFileFromStorage, getStorageBucketInfo } from './files.js';
...
export const performStorageHealthCheck = async () => {
  ...
  try {
    const bucketInfo = await getStorageBucketInfo();
    healthStatus.bucketExists = bucketInfo.exists;
    
    if (healthStatus.bucketExists) {
      healthStatus.canList = true; // If we can get bucket info, we can list.
      healthStatus.bucketInfo = bucketInfo;
