...
export const updateOrderStatus = async (orderId, status, paymentIntentId = null) => {
  ...
  console.log(`[UPDATE-ORDER] Updating order ${orderId} status to ${status}`)
  console.log(`[UPDATE-ORDER] Using client: ${supabaseAdmin ? 'admin (bypasses RLS)' : 'regular (subject to RLS)'}`)
  ...
  if (error) {
    console.error('[UPDATE-ORDER] Failed to update order:', {
      orderId,
      status,
      error: error.message,
      code: error.code
  ...
  console.log(`[UPDATE-ORDER] Successfully updated order ${orderId}`)
  return data
}
...
export const getOrderById = async (orderId) => {
  ...
  console.log(`[GET-ORDER] Fetching order ${orderId}`)
  console.log(`[GET-ORDER] Using client: ${supabaseAdmin ? 'admin (bypasses RLS)' : 'regular (subject to RLS)'}`)
  ...
  if (error) {
    ...
    console.error('[GET-ORDER] Failed to fetch order:', {
      orderId,
      error: error.message,
      code: error.code
    })
  ...
  console.log(`[GET-ORDER] Successfully fetched order ${orderId} with ${data.order_items?.length || 0} items`)
  return data
}
...
export const createDownloadLinks = async (orderItems, customerEmail) => {
  // Use admin client to bypass RLS policies for webhook and system operations
  const client = supabaseAdmin || supabase
  
  console.log(`[CREATE-DOWNLOADS] Creating download links for ${orderItems.length} items`)
  console.log(`[CREATE-DOWNLOADS] Using client: ${supabaseAdmin ? 'admin (bypasses RLS)' : 'regular (subject to RLS)'}`)
  ...
  console.log('[CREATE-DOWNLOADS] Download data:', downloads.map(d => ({
  ...
  if (error) {
    console.error('[CREATE-DOWNLOADS] Failed to create download links:', {
      error: error.message,
      code: error.code,
      details: error.details,
      hint: error.hint
  ...
  console.log(`[CREATE-DOWNLOADS] Successfully created ${data.length} download links`)
  return data
}
...
// PayPal Order Management Functions
export const updateOrderWithPayPalId = async (orderId, paypalOrderId) => {
  const client = supabaseAdmin || supabase
  console.log(`[UPDATE-ORDER-PAYPAL] Updating order ${orderId} with PayPal order ID: ${paypalOrderId}`)
  ...
  console.log(`[UPDATE-ORDER-PAYPAL] Successfully updated order ${orderId}`)
  return data
}

export const getOrderByPayPalId = async (paypalOrderId) => {
  const client = supabaseAdmin || supabase
  console.log(`[GET-ORDER-PAYPAL] Fetching order by PayPal ID: ${paypalOrderId}`)
  ...
  if (error && error.code === 'PGRST116') {
    console.log(`[GET-ORDER-PAYPAL] Order not found for PayPal ID: ${paypalOrderId}`)
    return null
  }
  
  console.log(`[GET-ORDER-PAYPAL] Successfully found order ${data.id} for PayPal ID: ${paypalOrderId}`)
  return data
}
  ...
  try {
    ...
    if (fileId) {
      ...
      if (fileType === 'pdf') {
        ...
        // CRITICAL FIX: Update the image URL to use the actual uploaded image
        // instead of the placeholder URL that was passed in
        if (fileUpload.storage_path) {
          const actualImageUrl = getFileStorageUrl(fileUpload.storage_path)
          console.log(`[PRODUCT-IMAGE] Replacing placeholder URL with actual image URL`)
          console.log(`[PRODUCT-IMAGE] Original: ${productData.image}`)
          console.log(`[PRODUCT-IMAGE] New URL: ${actualImageUrl}`)
